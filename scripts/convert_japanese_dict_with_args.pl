...
sub usage {
    print <<USAGE;
usage: $FindBin::Script [--cforms-file <path to cforms file>] [--pos-table-file <path to POS table file>] --prefix <output file prefix> [-n|--normal-dict <path to a dictionary for indeclinable words]... [-d|--declinable-dict <path to a dictionary for declinable words]... [-c|--concat-dict <path to a concatenation dictionary]...
USAGE
    exit(0);
}
...
sub main {
    ...
    for my $type(keys %{$output_dict{"output-types"}}) {
        ...
        open $output_dict{"output-types"}{$type}{"handle"}, ">", $filename.".temp" or die;
    }
    ...
    %output_set = ();
    @output_set{qw(segment postag morph)} = ();
    for my $file(@{$opts{"declinable-dict"}}) {
        open IN, "<", $file;
        while (<IN>) {
            chomp;
            my @F = split /\t/;
    ...
    for my $type(keys %{$output_types}) {
        ...
        if ($opts{"convert-dict"}) {
            system("$config_ref->{DICTIONARY_CONVERTER} -o $output_types->{$type}{'file'} < $output_types->{$type}{'file'}.temp");
            unlink $output_types->{$type}{"file"}.".temp";
        }
        else {
            rename $output_types->{$type}{"file"}.".temp", $output_types->{$type}{"file"};
        }
    }
}
